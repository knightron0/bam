...
wandb_project = 'sgd_iters_nanogpt_sweep'
...
use_sgd = True # if True, use SGD+Nesterov optimizer for 2D parameters and AdamW for others
sgd_lr = 0.01 # learning rate for SGD optimizer (CLI-tunable)
sgd_momentum = 0.95 # momentum for SGD optimizer
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
...
parser.add_argument('--sgd_lr', type=float, default=None, help='Learning rate for SGD optimizer (2D hidden parameters)')
parser.add_argument('--sgd_momentum', type=float, default=None, help='Momentum for SGD optimizer')
...
if cmd_args.sgd_lr is not None:
    sgd_lr = cmd_args.sgd_lr
if cmd_args.sgd_momentum is not None:
    sgd_momentum = cmd_args.sgd_momentum
...
if wandb_log:
    ...
    run_name_parts = [
        ...
        f"sgd_lr{sgd_lr:.0e}",
        f"sgd_momentum{sgd_momentum:.2f}",
    ]
...
if use_sgd:
    # Separate parameters: 2D+ for SGD+Nesterov, others for AdamW
    # Embeddings and final layer use AdamW, hidden matrices use SGD+Nesterov
    ...
    num_sgd_params = sum(p.numel() for p in hidden_matrix_params)
    num_adam_params = sum(p.numel() for p in embed_params + scalar_params + head_params)
    print(f"num SGD parameter tensors: {len(hidden_matrix_params)}, with {num_sgd_params:,} parameters")
    ...
    # Optimizer 2: SGD + Nesterov Momentum for 2D hidden parameters
    decay_params_2d = [p for p in hidden_matrix_params if p.dim() >= 2]
    nodecay_params_2d = [p for p in hidden_matrix_params if p.dim() < 2]
    optim_groups_2d = [
        {'params': decay_params_2d, 'weight_decay': weight_decay},
        {'params': nodecay_params_2d, 'weight_decay': 0.0}
    ]
    print(f"Optimizer 2 (SGD+Nesterov for 2D): LR={sgd_lr}, momentum={sgd_momentum}, weight_decay={weight_decay}, nesterov=True")
    optimizer2 = torch.optim.SGD(optim_groups_2d, lr=sgd_lr, momentum=sgd_momentum, weight_decay=weight_decay, nesterov=True)
...
while True:
    ...
    if isinstance(optimizer, list):
        ...
        # Apply SGD momentum warmup
        if use_sgd and len(optimizer) > 1:
            frac = min(iter_num / 300, 1)  # momentum warmup for SGD
            for group in optimizer[1].param_groups:
                group['momentum'] = (1 - frac) * 0.85 + frac * sgd_momentum
    ...
    if iter_num % eval_interval == 0:
        ...
        if wandb_log:
            ...
            if isinstance(optimizer, list):
                ...
                log_dict["lr/sgd"] = optimizer[1].param_groups[0]['lr']
                log_dict["momentum/sgd"] = optimizer[1].param_groups[0]['momentum']
            else:
                log_dict["lr"] = lr
            wandb.log(log_dict)
