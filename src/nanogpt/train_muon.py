...
wandb_project = 'muon_nanogpt_sweep'
...
use_muon = True # if True, use Muon optimizer for 2D parameters and AdamW for others
muon_lr = 0.01 # learning rate for Muon optimizer (CLI-tunable)
...
parser.add_argument('--muon_lr', type=float, default=None, help='Learning rate for Muon optimizer (2D hidden parameters)')
...
if cmd_args.muon_lr is not None:
    muon_lr = cmd_args.muon_lr
if cmd_args.muon_momentum is not None:
    muon_momentum = cmd_args.muon_momentum
...
if wandb_log:
    ...
    run_name_parts = [
        ...
        f"muon_lr{muon_lr:.0e}",
        f"muon_momentum{muon_momentum:.2f}",
    ]
...
class Muon(torch.optim.Optimizer):
    ...
    """
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, track_singulars=False):
    ...
    @torch.no_grad()
    def step(self):
        # Single GPU implementation (non-distributed)
        self.step_counter += 1
        should_track = self.track_singulars and (self.step_counter % 50 == 0)
        if should_track:
            print(f"[Muon] Tracking singulars at step {self.step_counter}, param_id_to_name has {len(self.param_id_to_name)} entries")
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                if should_track and p.ndim >= 2:
                    ...
                    U = v.detach().reshape(v.shape[0], -1) if v.ndim > 2 else v.detach()
                    try:
                        ...
                        if name is not None:
                            ...
                            print(f"[Muon] Warning: Parameter with id {id(p)} and shape {p.shape} not found in param_id_to_name")
                    except RuntimeError as e:
                        print(f"[Muon] Error computing SVD: {e}")
                        pass
...
if use_muon:
    # Separate parameters: 2D+ for Muon, others for AdamW
    # Embeddings and final layer use AdamW, hidden matrices use Muon
    ...
    num_muon_params = sum(p.numel() for p in hidden_matrix_params)
    num_adam_params = sum(p.numel() for p in embed_params + scalar_params + head_params)
    print(f"num Muon parameter tensors: {len(hidden_matrix_params)}, with {num_muon_params:,} parameters")
    ...
    # Optimizer 2: Muon for 2D hidden parameters
    print(f"Optimizer 2 (Muon for 2D): LR={muon_lr}, momentum={muon_momentum}, weight_decay={weight_decay}")
    optimizer2 = Muon(hidden_matrix_params, lr=muon_lr, momentum=muon_momentum, weight_decay=weight_decay)
...
while True:
    ...
    if isinstance(optimizer, list):
        ...
        if use_muon and len(optimizer) > 1:
            frac = min(iter_num / 300, 1)  # momentum warmup for muon
            optimizer[1].param_groups[0]['momentum'] = (1 - frac) * 0.85 + frac * muon_momentum
    ...
    if iter_num % eval_interval == 0:
        ...
        if wandb_log:
            ...
            if isinstance(optimizer, list):
                ...
                log_dict["lr/muon"] = optimizer[1].param_groups[0]['lr']
                log_dict["momentum/muon"] = optimizer[1].param_groups[0]['momentum']
            else:
                log_dict["lr"] = lr
            wandb.log(log_dict)
