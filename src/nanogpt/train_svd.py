...
wandb_project = 'svd2_nanogpt_sweep'
...
use_svd = True # if True, use SVD optimizer for 2D parameters and AdamW for others
svd_lr = 0.01 # learning rate for SVD optimizer (CLI-tunable)
svd_momentum = 0.95 # momentum for SVD optimizer
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
...
parser.add_argument('--svd_lr', type=float, default=None, help='Learning rate for SVD optimizer (2D hidden parameters)')
parser.add_argument('--svd_momentum', type=float, default=None, help='Momentum for SVD optimizer')
...
if cmd_args.svd_lr is not None:
    svd_lr = cmd_args.svd_lr
if cmd_args.svd_momentum is not None:
    svd_momentum = cmd_args.svd_momentum
...
if wandb_log:
    ...
    run_name_parts = [
        ...
        f"svd_lr{svd_lr:.0e}",
        f"svd_momentum{svd_momentum:.2f}",
    ]
...
def svd_update(grad, momentum, beta=0.95, ns_steps=5, nesterov=True):
...
class SVD(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        if should_track:
            print(f"[SVD] Tracking singulars at step {self.step_counter}, param_id_to_name has {len(self.param_id_to_name)} entries")
                ...
                if should_track and p.ndim >= 2:
                    ...
                    try:
                        ...
                        if name is not None:
                            ...
                            print(f"[SVD] Warning: Parameter with id {id(p)} and shape {p.shape} not found in param_id_to_name")
                    except RuntimeError as e:
                        print(f"[SVD] Error computing SVD: {e}")
                        pass
...
if use_svd:
    # Separate parameters: 2D+ for SVD, others for AdamW
    # Embeddings and final layer use AdamW, hidden matrices use SVD
    ...
    num_svd_params = sum(p.numel() for p in hidden_matrix_params)
    num_adam_params = sum(p.numel() for p in embed_params + scalar_params + head_params)
    print(f"num SVD parameter tensors: {len(hidden_matrix_params)}, with {num_svd_params:,} parameters")
    ...
    # Optimizer 2: SVD for 2D hidden parameters
    print(f"Optimizer 2 (SVD for 2D): LR={svd_lr}, momentum={svd_momentum}, weight_decay={weight_decay}")
    optimizer2 = SVD(hidden_matrix_params, lr=svd_lr, momentum=svd_momentum, weight_decay=weight_decay)
...
while True:
    ...
    if isinstance(optimizer, list):
        ...
        # Apply SVD momentum warmup
        if use_svd and len(optimizer) > 1:
            frac = min(iter_num / 300, 1)  # momentum warmup for svd
            optimizer[1].param_groups[0]['momentum'] = (1 - frac) * 0.85 + frac * svd_momentum
    ...
    if iter_num % eval_interval == 0:
        ...
        if wandb_log:
            ...
            if isinstance(optimizer, list):
                ...
                log_dict["lr/svd"] = optimizer[1].param_groups[0]['lr']
                log_dict["momentum/svd"] = optimizer[1].param_groups[0]['momentum']
            else:
                log_dict["lr"] = lr
            wandb.log(log_dict)
