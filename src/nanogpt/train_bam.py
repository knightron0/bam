...
wandb_project = 'bam_nanogpt_sweep'
...
use_bam = True # if True, use BAM optimizer for 2D parameters and AdamW for others
bam_lr = 0.01 # learning rate for BAM optimizer (CLI-tunable)
bam_momentum = 0.95 # momentum for BAM optimizer
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
...
parser.add_argument('--bam_lr', type=float, default=None, help='Learning rate for BAM optimizer (2D hidden parameters)')
parser.add_argument('--bam_momentum', type=float, default=None, help='Momentum for BAM optimizer')
...
if cmd_args.bam_lr is not None:
    bam_lr = cmd_args.bam_lr
if cmd_args.bam_momentum is not None:
    bam_momentum = cmd_args.bam_momentum
...
if wandb_log:
    ...
    run_name_parts = [
        ...
        f"bam_lr{bam_lr:.0e}",
        f"bam_momentum{bam_momentum:.2f}",
    ]
...
@torch.compile
def sink_normed(G, steps: int):
    #sinkhorn but p-normed
    assert G.ndim >= 2

    for _ in range(steps):
        #using p=2 norm -- 3 steps is enough
        if G.shape[-2] > G.shape[-1]:
            # tall matrix, row > columns, so i will normalize columns first
            ...
            # wide matrix, columns > rows, so i will normalize rows first
            G_norm_row = torch.linalg.vector_norm(G, ord=2, dim=-2, keepdim=True) + 1e-7
            G = G / G_norm_row
            G_norm_col = torch.linalg.vector_norm(G, ord=2, dim=-1, keepdim=True) + 1e-7
            G = G / G_norm_col
...
def bam_update(grad, momentum, beta=0.95, ns_steps=5, nesterov=True):
    momentum.lerp_(grad, 1 - beta)
    update = grad.lerp_(momentum, beta) if nesterov else momentum
    if update.ndim == 4: # for the case of conv filters
        update = update.reshape(len(update), -1)
    update = sink_normed(update, steps=1) # default to 3 iterations
...
class BAM(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        if should_track:
            print(f"[BAM] Tracking singulars at step {self.step_counter}, param_id_to_name has {len(self.param_id_to_name)} entries")
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                update = bam_update(p.grad, state["momentum_buffer"], beta=group["momentum"])
                if should_track and p.ndim >= 2:
                    ...
                    try:
                        ...
                        if name is not None:
                            ...
                            print(f"[BAM] Warning: Parameter with id {id(p)} and shape {p.shape} not found in param_id_to_name")
                    except RuntimeError as e:
                        print(f"[BAM] Error computing SVD: {e}")
                        pass
...
if use_bam:
    # Separate parameters: 2D+ for BAM, others for AdamW
    # Embeddings and final layer use AdamW, hidden matrices use BAM
    ...
    num_bam_params = sum(p.numel() for p in hidden_matrix_params)
    num_adam_params = sum(p.numel() for p in embed_params + scalar_params + head_params)
    print(f"num BAM parameter tensors: {len(hidden_matrix_params)}, with {num_bam_params:,} parameters")
    ...
    # Optimizer 2: BAM for 2D hidden parameters
    print(f"Optimizer 2 (BAM for 2D): LR={bam_lr}, momentum={bam_momentum}, weight_decay={weight_decay}")
    optimizer2 = BAM(hidden_matrix_params, lr=bam_lr, momentum=bam_momentum, weight_decay=weight_decay)
...
while True:
    ...
    if isinstance(optimizer, list):
        ...
        # Apply BAM momentum warmup
        if use_bam and len(optimizer) > 1:
            frac = min(iter_num / 300, 1)  # momentum warmup for bam
            optimizer[1].param_groups[0]['momentum'] = (1 - frac) * 0.85 + frac * bam_momentum
    ...
    if iter_num % eval_interval == 0:
        ...
        if wandb_log:
            ...
            if isinstance(optimizer, list):
                ...
                log_dict["lr/bam"] = optimizer[1].param_groups[0]['lr']
                log_dict["momentum/bam"] = optimizer[1].param_groups[0]['momentum']
            else:
                log_dict["lr"] = lr
            wandb.log(log_dict)
